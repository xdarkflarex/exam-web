...
import { getTheoryById, updateTheory, deleteTheory } from '@/lib/theories/actions'
import { useRouter, useParams } from 'next/navigation'
import Link from 'next/link'
import {
  Save, ArrowLeft, Eye, Edit3, Star, Trash2, BookOpen,
  GitBranch, X
} from 'lucide-react'
...
export default function EditTheoryPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (theory) {
        ...
        setContentMd(theory.content_md || '')
        setSectionId(theory.section_id)
        setDifficultyLevel(theory.difficulty_level)
        setIsPublished(theory.is_published)
      }
      ...
      setMessage({ type: 'error', text: 'Không tìm thấy bài lý thuyết' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Đã cập nhật bài lý thuyết!' })
    } catch (err: any) {
      console.error('Save error:', err)
      setMessage({ type: 'error', text: err?.message || 'Lỗi khi cập nhật' })
    } finally {
      setSaving(false)
    }
  }
  ...
  if (loading) {
    return (
      <>
        <AdminHeader title="Chỉnh sửa lý thuyết" />
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              <Link
                href={`/admin/theories/${theoryId}/edges`}
                className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center gap-2"
              >
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition-colors flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-md w-full border border-slate-200 dark:border-slate-700 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Xác nhận xóa</h3>
              <button
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              Bạn có chắc chắn muốn xóa bài lý thuyết <strong className="text-slate-800 dark:text-slate-200">&quot;{title}&quot;</strong>? Hành động này không thể hoàn tác.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50"
              >
                {deleting ? (
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                ) : (
