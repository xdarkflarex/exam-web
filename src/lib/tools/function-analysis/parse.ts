/**
 * Đọc hàm số học sinh gõ (hoặc dán từ đề) thành phân thức N(x)/D(x) chính xác,
 * rồi xếp vào một trong các dạng SGK Toán 12 công cụ khảo sát được.
 *
...
function tokenize(s: string): Token[] | string {
  ...
  while (i < s.length) {
    ...
    return `Không hiểu ký tự “${ch}”. Chỉ dùng biến x, số, + − * / ^ và dấu ngoặc.`
  }
  return out
}
...
class Parser {
  ...
  private power(): Rat {
    ...
    if (t?.t === 'num') {
      ...
      if (!this.isOp(')')) throw new Error('Số mũ phải là số tự nhiên, ví dụ x^3.')
      this.pos++
    }
    if (!exp || !exp.isInteger() || exp.sign() < 0 || exp.toNumber() > 8) {
      throw new Error('Số mũ phải là số tự nhiên từ 0 đến 8, ví dụ x^3.')
    }
  ...
  private atom(): Rat {
    ...
    throw new Error(`Thiếu số hoặc x trước “${t.v}”.`)
  }
}
...
export function parseFunction(src: string): ParseResult {
  ...
  if (!text) return { ok: false, error: 'Nhập hàm số, ví dụ x^3 - 3x^2 + 2.' }
  const tokens = tokenize(text)
  if (typeof tokens === 'string') return { ok: false, error: tokens }
  let rat: Rat
  try {
    rat = tidy(new Parser(tokens).parse())
  } catch (e) {
